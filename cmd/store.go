...
func (es *EventStore) sync(obj interface{}) {
	...
	fmt.Printf("\nNew event found %s/%s", event.InvolvedObject.Namespace, event.InvolvedObject.Name)
	eventCleaning(event)
...
func (es *EventStore) Scrap(ch chan<- prometheus.Metric) {
	for key, isHappening := range es.backoff.AllKeysStateSinceUpdate(time.Now()) {
		log.Infof("Scrap new event: %s", key)
		obj, exists, err := es.eventStore.GetByKey(key)
		if err != nil {
			log.Infof("Scrap key not found: %s", key)
			continue
		} else if !exists {
			log.Errorf("event not found: %s", key)
			continue
		}
		event := obj.(*core_v1.Event)
